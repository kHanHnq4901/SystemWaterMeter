  ...
  try {
    ...
    if (code === 0 && data?.list) {
      meterOptions.value = data.list.map((m: any) => ({
        ...
        zone: m.regionName ?? t("report.pressure.unzoned")
      }));
...
async function handleSearch() {
  ...
  try {
    ...
    (data as any[]).forEach((r: any) => {
      ...
      rowMap[period][`${meterNo}_avg`] = +(r.avgPressure ?? 0).toFixed(2);
      rowMap[period][`${meterNo}_min`] = +(r.minPressure ?? 0).toFixed(2);
      rowMap[period][`${meterNo}_max`] = +(r.maxPressure ?? 0).toFixed(2);
    });
...
const meterStats = computed(() =>
  selectedMeters.value.map((id, idx) => {
    ...
    return {
      ...
      min: mins.length ? +Math.min(...mins).toFixed(2) : 0,
      max: maxs.length ? +Math.max(...maxs).toFixed(2) : 0
    };
  })
);
...
function renderChart() {
  ...
  setOptions({
    ...
    yAxis: { type: "value", name: "bar", min: 0, axisLabel: { formatter: (v: number) => v.toFixed(1) } },
  ...
  <div class="main p-4 space-y-4">
    ...
    <div class="bg-bg_color rounded-lg p-4 flex flex-wrap items-end gap-3">
      <div class="flex flex-col gap-1">
        <span class="text-xs text-gray-500">{{ t("report.pressure.selectMeters") }}</span>
        <el-select
          v-model="selectedMeters"
          multiple filterable collapse-tags collapse-tags-tooltip
          :placeholder="t('report.pressure.selectMetersPlaceholder')"
          style="width: 300px"
          :max-collapse-tags="2"
      ...
      <div class="flex flex-col gap-1">
        <span class="text-xs text-gray-500">{{ t("report.pressure.viewBy") }}</span>
        <el-radio-group v-model="viewMode" @change="handleSearch">
          <el-radio-button value="hour">{{ t("report.pressure.byHour") }}</el-radio-button>
          <el-radio-button value="day">{{ t("report.pressure.byDay") }}</el-radio-button>
        </el-radio-group>
      </div>

      <div v-if="viewMode === 'hour'" class="flex flex-col gap-1">
        <span class="text-xs text-gray-500">{{ t("report.pressure.date") }}</span>
        <el-date-picker v-model="selectedDate" type="date" :placeholder="t('report.pressure.date')"
          format="DD/MM/YYYY" value-format="YYYY-MM-DD" style="width: 150px" />
      </div>
      <div v-else class="flex flex-col gap-1">
        <span class="text-xs text-gray-500">{{ t("report.pressure.dateRange") }}</span>
      ...
      <el-button type="primary" :loading="loading" @click="handleSearch">{{ t("report.pressure.viewData") }}</el-button>
      <el-button type="success" class="ml-auto">
        <template #icon><i class="i-ri:download-line" /></template>
        {{ t("report.pressure.exportExcel") }}
      </el-button>
    </div>

    <el-alert v-if="!selectedMeters.length"
      :title="t('report.pressure.selectWarning')"
    ...
    <div v-if="selectedMeters.length && timeLabels.length" class="bg-bg_color rounded-lg p-4">
      <div class="font-semibold text-sm mb-3">
        {{ t("report.pressure.pressureChart") }}
        <span class="text-gray-400 font-normal text-xs ml-2">
          {{ viewMode === 'hour' ? `${t('report.pressure.byHour')} – ${selectedDate}` : `${t('report.pressure.byDay')} – ${dateRange[0]} → ${dateRange[1]}` }}
        </span>
      </div>
    ...
    <div v-if="selectedMeters.length && pivotRows.length" class="bg-bg_color rounded-lg p-4">
      ...
      <el-table :data="pagedRows" size="small" stripe border
        ...
        <el-table-column prop="time" :label="t('report.pressure.time')" width="100" fixed="left" align="center"
